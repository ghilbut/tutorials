...
public:
	explicit AudioDecoder(PacketQueue& q)
		...
		audio_buffer_size_(0), audio_buffer_index_(0)
		//audio_packet_data_(0), audio_packet_size_(0)
	{
	...
	void DoCallback(Uint8* stream, int len)
	{
		...
		while (len > 0) {
			if (audio_buffer_index_ >= audio_buffer_size_) {
				...
				audio_buffer_size_ = DecodeAudio(codec_ctx, audio_buffer_);
				audio_buffer_index_ = 0;
			}

			if (audio_buffer_size_ > 0) {
				const size_t bytes_stream = std::min(audio_buffer_size_ - audio_buffer_index_, static_cast<size_t>(len));
				memcpy(stream, audio_buffer_ + audio_buffer_index_, bytes_stream);

				len -= bytes_stream;
				stream += bytes_stream;
				audio_buffer_index_ += bytes_stream;
			}
		}
...
private:
	...
	uint8_t     audio_buffer_[AVCODEC_MAX_AUDIO_FRAME_SIZE];
	size_t      audio_buffer_size_;
	size_t      audio_buffer_index_;
	AVPacket    audio_packet_;
	//uint8_t*    audio_packet_data_;
	//size_t      audio_packet_size_;
};
