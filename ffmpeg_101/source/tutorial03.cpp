...
void audio_callback(void* userdata, Uint8* stream, int len) 
{
	...
	static uint8_t buffer[AVCODEC_MAX_AUDIO_FRAME_SIZE];
	...
	while (len > 0) {
		if (buffer_index >= buffer_size) {
			...
			buffer_size = DecodeAudio(codec_ctx, buffer);
			buffer_index = 0;
		}

		if (buffer_size > 0) {
			const size_t bytes_stream = std::min(buffer_size - buffer_index, static_cast<size_t>(len));
			memcpy(stream, static_cast<uint8_t*>(buffer) + buffer_index, bytes_stream);

			len -= bytes_stream;
			stream += bytes_stream;
			buffer_index += bytes_stream;
		}
	}
